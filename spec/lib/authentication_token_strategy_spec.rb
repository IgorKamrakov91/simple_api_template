...
RSpec.describe AuthenticationTokenStrategy, type: :model do
  ...
  let(:env) {{ 'HTTP_X_USER_EMAIL' => user.email, 'HTTP_X_AUTH_TOKEN' => authentication_token.body }}
  ...
  describe '#authenticate!' do
    ...
    context 'with invalid user' do
      before {allow(subject).to receive(:env)
        .and_return({ 'HTTP_X_USER_EMAIL' => 'invalid@email', 'HTTP_X_AUTH_TOKEN' => 'invalid-token'})
      }
      ...
      it 'fails' do
        ...
        expect(TokenIssuer).not_to receive(:success!)
    ...
    context 'with invalid token' do
      before {allow(subject).to receive(:env)
        .and_return({"HTTP_X_USER_EMAIL" => user.email, "HTTP_X_AUTH_TOKEN" => 'invalid-token'})
      }
